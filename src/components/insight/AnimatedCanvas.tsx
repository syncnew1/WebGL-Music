...
export default function AnimatedCanvas<T, U, V extends keyof ContextTypeMap>(props: {
  ...
  useEffect(() => {
    ...
    const resizeObserver = new win.ResizeObserver(() => {
      const canvas = canvasRef.current
      if (!canvas) return
      const win2 = canvas.ownerDocument.defaultView
      if (!win2) return
      updateResolution(canvas, win2)
      const context = canvas.getContext(contextType) as ContextTypeMap[V] | null
  ...
  return <canvas ref={canvasRef} style={{ ...(style || {}), ...(isEnabled ? {} : { visibility: 'hidden' }) }} />
}